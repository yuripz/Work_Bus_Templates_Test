...
-->
					<orderAttributes>
						...
						<attribute name="regionalFlag">
							<xsl:if test="(not(OrderOMSParam/regionalFlag)) or OrderOMSParam/regionalFlag='' or OrderOMSParam/regionalFlag='0' ">false</xsl:if>
							<xsl:if test="OrderOMSParam/regionalFlag='1'">true</xsl:if>
						</attribute>
					</orderAttributes>
					<orderDate>
					...
					<orderParties>
						...
						<orderParty>
							...
							<partyAttributes>
								...
								<xsl:choose>
									<xsl:when test="/submitOrderRequest/selfInstallation!=''">
										<attribute name="selfInstallation">possible</attribute>
									</xsl:when>
									<xsl:otherwise>
										<attribute name="selfInstallation">false</attribute>
									</xsl:otherwise>
								</xsl:choose>
							</partyAttributes>
						</orderParty>
						...
						<xsl:if test="(/submitOrderRequest/OrderOMSContact/OrderContactPhone != '') or (/submitOrderRequest/OrderOMSContact/OrderContactEmail != '') ">
							<orderParty>
								<partyRole>CONTACT</partyRole>
								<partyId>
									<xsl:value-of select="/submitOrderRequest/CRMCustomer/CUSTOM_ID"/>-CLIENTAGENT</partyId>
						...
						<xsl:if test="not (/submitOrderRequest/OrderOMSSpecialities/WHOLESALE/fio = '' )">
							<orderParty>
								<partyRole>CONTACT</partyRole>
								<partyId>
									<xsl:value-of select="/submitOrderRequest/CRMOrderId"/>-WHOLESALE</partyId>
								<partyName>
									<xsl:value-of select="/submitOrderRequest/OrderOMSSpecialities/WHOLESALE/fio"/>
								</partyName>
								<partyAttributes>
									<attribute name="phone">
										<xsl:value-of select="/submitOrderRequest/OrderOMSSpecialities/WHOLESALE/phone"/>
									</attribute>
									<attribute name="email">
										<xsl:value-of select="/submitOrderRequest/OrderOMSSpecialities/WHOLESALE/email"/>
									</attribute>
									<attribute name="speciality">WHOLESALE</attribute>
								</partyAttributes>
							</orderParty>
						</xsl:if>
					</orderParties>
