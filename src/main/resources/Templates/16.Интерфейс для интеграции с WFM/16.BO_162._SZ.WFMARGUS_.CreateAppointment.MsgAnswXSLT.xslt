...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="createAppointmentResponse">
				<Confirmation>
					<ResultCode>
						<xsl:value-of select="/createAppointmentResponse/orderResult/orderResultCode"/>
					</ResultCode>
					<Message>
						<xsl:value-of select="/createAppointmentResponse/orderResult//orderResultText"/>
					</Message>
					<DetailList>
						<xsl:copy-of select="createAppointmentResponse"/>
					</DetailList>
					<Next>RESOUT</Next>
				</Confirmation>
			</xsl:when>
