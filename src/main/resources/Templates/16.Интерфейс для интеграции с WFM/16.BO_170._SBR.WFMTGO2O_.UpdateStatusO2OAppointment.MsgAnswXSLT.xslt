...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="updateAppointmentResponse">
				<Confirmation>
					<ResultCode>
						<xsl:value-of select="/updateAppointmentResponse/orderResult/orderResultCode"/>
					</ResultCode>
					<Message>
						<xsl:value-of select="/updateAppointmentResponse/orderResult//orderResultText"/>
					</Message>
					<DetailList>
						<xsl:copy-of select="updateAppointmentResponse"/>
					</DetailList>
					<Next>RESOUT</Next>
				</Confirmation>
			</xsl:when>
