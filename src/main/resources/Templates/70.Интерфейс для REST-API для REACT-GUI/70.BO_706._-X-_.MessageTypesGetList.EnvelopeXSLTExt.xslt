...
from artx_Proj.Message_Types
 where 1=1 <xsl:value-of select="$TableListFilter"/>
...
FETCH NEXT <xsl:value-of select="$PageSize"/> ROWS only</PSTMT>
						...
						<TotalStatement type="select" snum="1">select count(*) from artx_Proj.Message_Types where 1=1 <xsl:value-of select="$TableListFilter"/>
						</TotalStatement>
					</SQLRequest>
				</xsl:for-each>
			</xsl:for-each>
