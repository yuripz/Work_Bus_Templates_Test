				...
				<xsl:for-each select="Parametrs">
					...
					<SQLRequest>
						...
						<SQLStatement type="select" snum="1">
							<PSTMT>select d.RowId, d.queue_id, d.tag_id, d.tag_value, d.tag_num, d.tag_par_num from artx_Proj.MESSAGE_QUEUEDET d where 1=1 <xsl:value-of select="$TableListFilter"/> Order by tag_par_num, tag_num
OFFSET <xsl:value-of select="$FirstRecordNo"/> ROWS FETCH NEXT <xsl:value-of select="$PageSize"/> ROWS only</PSTMT>
						</SQLStatement>
						<TotalStatement type="select" snum="1">select count(*) from artx_Proj.MESSAGE_QUEUEdet where 1=1 <xsl:value-of select="$TableListFilter"/>
						</TotalStatement>
					</SQLRequest>
				</xsl:for-each>
			</xsl:for-each>
